...
// Suppress unet deprecation warning.
...
namespace Smooth
{
    ...
    public class SmoothSync : NetworkBehaviour
	{
        ...
        /// <summary> Made to reset back to zero when float value gets too high / imprecise after running for 131072 (2^17) seconds. </summary>
        [NonSerialized]
        public float currentLocalTime;
        /// <summary> Reset currentLocalTime back to zero when float value gets this high. </summary>
        float maxCurrentLocalTime = 131072.0f;
        ...
        void Update()
        {
            currentLocalTime += Time.deltaTime;
        ...
        void FixedUpdate()
        {
            ...
            // and force a State send so clients know to reset time.
            if (currentLocalTime > maxCurrentLocalTime)
            {
                currentLocalTime = 0.0f;
                lastTimeStateWasSent -= maxCurrentLocalTime;
                latestAuthorityChangeZeroTime -= maxCurrentLocalTime;
                lastTimeOwnerTimeWasSet -= maxCurrentLocalTime;
...
#region Internal stuff
        ...
        void sendState()
        {
            ...
            if (currentLocalTime - lastTimeStateWasSent < GetNetworkSendInterval() && !forceStateSend) return;
            ...
            if (restStatePosition == RestState.JUST_STARTED_MOVING ||
                restStateRotation == RestState.JUST_STARTED_MOVING)
            {
                sendingTempState.state.ownerTimestamp = currentLocalTime - Time.deltaTime;
                if (restStatePosition != RestState.JUST_STARTED_MOVING)
                {
            ...
            lastTimeStateWasSent = currentLocalTime;
        ...
        void applyInterpolationOrExtrapolation()
        {
            ...
            // Don't extrapolate if we just changed authority.
                ...
                currentLocalTime - latestAuthorityChangeZeroTime > interpolationBackTime* 2.0f) ||
        ...
        public void addState(State state)
        {
            ...
            if (stateCount > 1 && state.ownerTimestamp <= stateBuffer[0].ownerTimestamp && 
                !(stateBuffer[0].ownerTimestamp - state.ownerTimestamp > maxCurrentLocalTime * .95f))
            {
                return;
            }

            // A way to handle time resetting so we know to change the times of States already in the buffer.
            if (stateCount > 1 && stateBuffer[0].ownerTimestamp - state.ownerTimestamp > maxCurrentLocalTime * .95f)
            {
                for (int i = 0; i < stateCount; i++)
                {
                    stateBuffer[i].ownerTimestamp -= maxCurrentLocalTime;
                }
            }
        ...
        public void teleportOwnedObjectFromOwner()
        {
            ...
            if (NetworkServer.active)
            {
                RpcTeleport(getPosition(), getRotation().eulerAngles, getScale(), currentLocalTime);
                
            }
            else if (hasAuthority)
            {
                CmdTeleport(getPosition(), getRotation().eulerAngles, getScale(), currentLocalTime);
            }            
        }
        /// <summary>
        /// Teleport the object, the transform will not be interpolated on non-owners.
        ...
        {
            var target = NetworkServer.FindLocalObject(theNetID.netId);
            if (target == null)
            {
            ...
            var targetSmooth = target.GetComponent<SmoothSync>();

            if (targetSmooth == null) return;

            var childObjectSmoothSyncs = targetSmooth.childObjectSmoothSyncs;
...
#region Sync Properties
        ...
        void SendStateToNonOwners(MessageBase state)
        {
            foreach (var kv in netID.observers)
            {
                ...
                if (conn != null && conn != netID.clientAuthorityOwner && conn.hostId != -1 && conn.isReady)
                {
                    ...
                    conn.Send(MsgType.SmoothSyncFromServerToNonOwners, state);
                }
            }
        }
        ...
        static void HandleSyncFromOwnerToServer(NetworkMessage msg)
        {
                ...
                networkState.smoothSync.validateStateMethod(networkState.state, networkState.smoothSync.latestValidatedState))
            {
                ...
                networkState.smoothSync.latestValidatedState.receivedOnServerTimestamp = networkState.smoothSync.currentLocalTime;
        ...
        public void checkIfOwnerHasChanged(State newState)
        {
            ...
            {                
                ...
                latestAuthorityChangeZeroTime = currentLocalTime;
                stateCount = 0;
...
#region Time stuff
        ...
        public float approximateNetworkTimeOnOwner
        {
            get
            {
                return _ownerTime + (currentLocalTime - lastTimeOwnerTimeWasSet);
            }
            set
            {
                _ownerTime = value;
                lastTimeOwnerTimeWasSet = currentLocalTime;
            }
        }
        /// <summary> Used to know when the owner has last changed. </summary>
        ...
        void adjustOwnerTime() 
        {
            ...
            {
                firstReceivedMessageZeroTime = currentLocalTime;
            }            
